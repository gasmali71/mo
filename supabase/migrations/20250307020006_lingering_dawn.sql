...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    AND policyname = 'Admins can read system logs'
  ) THEN
  ...
  IF NOT EXISTS (
    ...
    WHERE schemaname = 'public' 
    AND tablename = 'system_logs' 
    AND policyname = 'Service role can manage system logs'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE schemaname = 'public' 
    AND tablename = 'feature_registry' 
    AND policyname = 'Users can read feature registry'
  ) THEN
